...
function PpBoPage() {
  ...
  const [alpp, setAlpp] = useState([]);

  // Fetch data from API
  const fetchFireSafetyData = async () => {
    try {
      const response = await axios.get("http://localhost:8080/alppApi");
      setAlpp(response.data.alpp);
      console.log(response.data.alpp);
    } catch (error) {
